...
locals {
  ...
  record_intranet_ip = var.create_private_dns_record ? concat(tencentcloud_kubernetes_cluster.cluster.*.pgw_endpoint, [""])[0] : var.record_intranet_ip
}
...
resource "tencentcloud_kubernetes_native_node_pool" "native_node_pools" {
  ...
  native {
    ...
    management {
      ...
      hosts       = try(each.value.management.hosts, [])
      kernel_args = try(each.value.management.kernel_args, [])
    }
